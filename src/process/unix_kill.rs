...
//!   returns `Err` — never a panic, and deliberately never `Ok(None)`: a probe
//!   that could not RUN leaves the port's state UNKNOWN, and reporting that as
//!   "nothing is listening" is what makes a caller conclude there is nothing to
//!   kill. Callers record it as a non-run rung and fall back to the port-free
//!   confirmation loop.
...
/// Return every distinct PID found LISTENING on `port` via `lsof`.
///
/// Three-state on purpose, mirroring the Windows `netstat` probe:
/// `Ok(vec![])` means **the probe ran and the port is idle**; `Err` means the
/// probe could NOT run (`lsof` absent or unable to execute) and the port's
/// state is therefore UNKNOWN. Collapsing the second into the first is how a
/// probe that never ran comes to read as "nothing is listening" — and then as
/// "nothing to kill".
async fn find_pids_on_port(port: u16) -> anyhow::Result<Vec<u32>> {
    let output = Command::new("lsof")
        ...
        .map_err(|e| {
            debug!(
                "find_pids_on_port: lsof unavailable for port {}: {}",
                port, e
            );
            anyhow::anyhow!("lsof could not be run for port {}: {}", port, e)
        })?;

    // `lsof -t` exits 1 when it simply matched nothing — that is a RAN probe
    // reporting an idle port, not a failure. Any other non-zero status means
    // lsof itself failed and the answer is unknown.
    let code = output.status.code();
    if !output.status.success() && code != Some(1) {
        anyhow::bail!("lsof exited with {:?} probing port {}", code, port);
    }
...
/// Return the PID of the first process LISTENING on `port`. See
/// [`find_pids_on_port`] for why `Err` is not `Ok(None)`.
...
/// one kill landed. Propagates the probe failure as `Err` so a caller that
/// records what it actually did can distinguish "the port was idle" from
/// "we could not find out" (`stop_ledger`).
