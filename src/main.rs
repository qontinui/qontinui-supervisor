...
async fn main() -> anyhow::Result<()> {
    ...
    // Warm the `GET /builds` meta cache (slot freshness + LKG origin/main
    // drift) once at boot so the first poll after startup already has data.
    // Thereafter `GET /builds` refreshes it in the background when the
    // snapshot ages past its TTL — the endpoint itself never waits on git.
...
async fn reap_stale_test_runners(state: state::SharedState) {
    ...
    loop {
        ...
        let purged =
            crate::routes::runners::purge_stale_test_runners_core(&state, true, None).await;
        if !purged.is_empty() {
            info!(
